# path_curve_edit.gd
# Ajoute le mode courbe (Shift+clic sur segment) en Edit Points mode.
#
# Utilisation :
...
const _META_KEY = "PathCurveEditListener"
...
func cleanup() -> void:
	...
	_overlay_line = null
	print("[PathCurveEdit] Cleaned up")
...
func _install_listener():
	if Engine.has_meta(_META_KEY):
		...
		if is_instance_valid(old):
			...
			old.queue_free()
	var node = Node.new()
	node.name = "PathCurveEditListener"
	var s = GDScript.new()
...
func _is_edit_points_mode() -> bool:
	...
	var active = editor.get("ActiveTool")
	if active == null or not active.has_method("get_ActivePath"): return false
	# Vérifier le bouton UI EditPoints du PathTool
	var tools = _g.Editor.get("Tools")
	if tools == null: return false
	var pt = tools.get("PathTool")
	if pt == null: return false
	var btn = pt.get("EditPoints")
	if btn == null: return false
	return btn.get("pressed")


# ── Accès aux Pathways via Level ──────────────────────────────────────────────

func _get_all_pathways() -> Array:
	if _g == null: return []
	var world = _g.get("World")
	if world == null: return []
	var level = world.call("GetCurrentLevel")
	if level == null: return []
	var pathways_node = level.get("Pathways")
	if pathways_node == null: return []
	if not pathways_node.has_method("get_children"): return []
	var result = []
	for child in pathways_node.get_children():
		# Seuls les vrais Pathways ont GlobalEditPoints
		if child.get("GlobalEditPoints") == null: continue
		result.append(child)
	return result
...
func _read_global_pts(path) -> Array:
...
func _write_pts(path, pts: Array):
	var was_loop = path.get("loop")
	# Dans le cas wrappé, pts[0] = original_pts[bi] ≠ path.global_position.
	# DD calcule les coords locales comme (world_pt - path.global_position).
	# Pour que local[0] = (0,0), on déplace le path vers pts[0] avant SetEditPoints.
	...
	# Ne pas appeler UpdatePath sur PathTool — ça déclenche UpdateSelection
	# en C# qui crash sur un InvalidCastException.
...
func _find_path_and_segment(mouse_w: Vector2) -> Array:
	# Retourne [path, pts, anchor_a_idx, anchor_b_idx] ou []
	# Expansion depuis le segment le plus proche, stoppée uniquement par
	# MAX_STEP_ANGLE (vraie ancre angulaire). Pas de limite cumulative —
	# les grands arcs (>90°, >180°) sont détectés comme un seul segment.
	var best_path  = null
	var best_pts   = []
	var best_ai    = -1
	var best_bi    = -1
	...
	for pw in _get_all_pathways():
		...
		var n = pts.size()
		var is_closed = pw.get("loop") == true

		# Trouver le segment le plus proche (segment de fermeture DA inclus)
		var closest_i = -1
		var closest_d = CLICK_THRESHOLD
		var seg_count = n - 1 + (1 if is_closed else 0)
		...
		# Expansion vers l'arrière : s'arrête sur une vraie ancre (angle > MAX_STEP_ANGLE)
		var ai = closest_i
		var steps_back = 0
		while steps_back < n - 2:
			var prev_i = ai - 1
			if prev_i < 0:
				if is_closed: prev_i = n - 1
				else: break
			var next_i = ai + 1
			if next_i >= n: next_i = 0 if is_closed else n - 1
		...
		# Expansion vers l'avant : s'arrête sur une vraie ancre (angle > MAX_STEP_ANGLE)
		var bi = closest_i + 1
		if is_closed: bi = bi % n
		else: bi = min(bi, n - 1)
		var steps_fwd = 0
		while steps_fwd < n - 2:
			var next_i = bi + 1
			...
			var prev_i = bi - 1
			if prev_i < 0: prev_i = n - 1 if is_closed else 0
			var dc = pts[bi] - pts[prev_i]
			var dn = pts[next_i] - pts[bi]
			if dc.length() < 0.01 or dn.length() < 0.01: break
...
func _make_curve_pts(a: Vector2, b: Vector2, m: Vector2) -> Array:
	# Bézier quadratique passant par m à t=0.5
	var ctrl = 2.0 * m - 0.5 * (a + b)
	var pts = []
	for i in range(1, 18):
		var t = float(i) / 17.0   # t = 1/17 .. 17/17=1.0 → endpoint inclus
		pts.append((1.0-t)*(1.0-t)*a + 2.0*(1.0-t)*t*ctrl + t*t*b)
	return pts


func _build_pts(original: Array, ai: int, bi: int, curve_pts: Array) -> Array:
	# curve_pts inclut maintenant l'endpoint (bi) → pas de doublon à rajouter.
	var out = []
	if ai <= bi:
		for i in range(ai + 1):
...
func _get_preview_line(path) -> Node:
	...
	if not path.has_method("get_children"): return null
	for child in path.get_children():
		if child.get_class() == "Line2D" and not child.get("super"):
			return child
	return null
...
func _try_start_curve(mouse_w: Vector2) -> bool:
	var found = _find_path_and_segment(mouse_w)
	if found.empty():
		print("[CurveEdit] aucun segment proche (threshold=%d)" % int(CLICK_THRESHOLD))
		return false
	...
	print("[CurveEdit] mode courbe — segment [%d → %d]" % [_anchor_a_idx, _anchor_b_idx])
	return true
...
func _try_flatten(mouse_w: Vector2) -> bool:
	var found = _find_path_and_segment(mouse_w)
	if found.empty():
		print("[CurveEdit] flatten: aucun segment proche")
		return false
	var pw   = found[0]
	var pts  = found[1]
	var ai   = found[2]
	...
	print("[CurveEdit] segment [%d → %d] aplati" % [ai, bi])
	return true
...
func _flatten_pts(original: Array, ai: int, bi: int) -> Array:
	...
	if ai <= bi:
		...
		# loop=true ferme DA en ligne droite automatiquement — pas besoin de pts[bi] en fin.
		for i in range(bi, ai + 1):
			out.append(original[i])
	return out
...
func _confirm_curve():
	...
	# the path. Snapshot them now, before we clear state, so we can pair
	...
	_edit_path    = null
	_original_pts = []
	print("[CurveEdit] courbe confirmée")
...
func _cancel_curve():
	...
	_edit_path    = null
	_original_pts = []
	print("[CurveEdit] courbe annulée")
...
func update(_delta):
	...
	if not _debug_loop_printed:
		var pws = _get_all_pathways()
		if not pws.empty():
			var pw = pws[0]
			_debug_loop_printed = true
			var methods = []
	...
	if _is_edit_points_mode() and Input.is_key_pressed(KEY_SHIFT):
		...
		if not found.empty():
			...
			if pw != _hover_path or ai != _hover_ai or bi != _hover_bi or _state == State.HOVER and is_alt or _state == State.FLATTEN_HOVER and not is_alt:
				_clear_hover()
				_hover_path = pw
				_hover_ai   = ai
				_hover_bi   = bi
...
# update loop has written the final curve into the path) and from
...
func _load_record_script() -> void:
	...
	if _PointsRecordScript == null:
		print("[CurveEdit] WARN: library/points_history_record.gd not found; undo disabled")
